...
module.exports.weapon = function (req, res) {
    Product.find({ cateProduct: 'Weapon' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1) {
                if (err) {
                    ...
                    res.render('./paperuser/weapon', { listproduct: data, listdata: data1 })
                }
            });
        }
    })
...
module.exports.char = function (req, res) {
    Product.find({ cateProduct: 'Char' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1){
                if (err) {
                    ...
                    res.render('./paperuser/char', { listproduct: data, listdata:data1 });
                }
            })
        }
    })
